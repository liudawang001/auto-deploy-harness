...
class HuggingFaceDownloader(ResumableDownloadMixin):
    ...
    def _list_files(self, asset: ModelAsset) -> List[Dict]:
        ...
        for item in data:
            ...
            files.append({
                ...
                "etag": item.get("oid") or item.get("blob_id") or item.get("lfs", {}).get("oid"),
    ...
    def download_plan(self, repo_id, revision, plan, cache_path, progress_callback=None, disk_safety_ratio=1.2):
        ...
        Does NOT self-scan the source; it consumes exactly plan.files.
        Returns a dict with ``status`` in {complete, integrity_failed,
        insufficient_disk, download_failed}.
        """
        asset = ModelAsset(
            asset_id="huggingface:%s" % repo_id,
            source="huggingface",
            repo_id=repo_id,
            revision=revision,
        )
